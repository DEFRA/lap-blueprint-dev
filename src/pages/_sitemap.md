...
For: Engineers building the live AIM Blueprint website
Owner: PM maintains this alongside the AIM Blueprint itself

# **Purpose and How to Use This**

The AIM Blueprint currently exists as a set of working content files. This document is the content model for turning it into an interactive website, it defines, page-type by page-type, exactly what fields and sections belong on each page, so a new delivery team member with no technical background can land on any page and know: what this is, what to do, and who to ask if it's not enough.
...
1. Do not invent a 7th navigation section for "the 7 blockers or specific to backlog items". These blockers once resolved and documented will feed into the sitemap and pages below which in turn keeps the site's structure identical to the AIM Blueprint's existing 5/6 part structure.
...
This matches the existing AIM Blueprint structure exactly. No new top-level sections have been added.
...
- Mission statement (one sentence summarising the AIM Blueprint's purpose)
